...
console.log('panel utils loaded');
...
function create(dimensions, panelOptions) {
  ...
  console.log('_create returned a panel: ', sdkPanel);
  console.log('about to wait for the panel to be styled');

  // the panel fires this event after it's applied styles.
  events.on('DOMContentLoaded', ({subject}) => {
    console.log('we got _some_ DOMContentLoaded event. subject is: ', subject, ' and sdkPanel is ', sdkPanel);

    console.log('about to add a popupshowing listener. does the panel even exist? ', getActiveView(sdkPanel));
    // Add a capturing popupshowing listener that does all the XUL customization.
    getActiveView(sdkPanel).addEventListener('popupshowing', updatePanel, true);

    // Rather than wait for all this stuff to load, just use a popupshown listener
    // to toggle the whenPanelReady calls.
    getActiveView(sdkPanel).addEventListener('popupshown', whenPanelReady.ready);
    console.log('popupshowing and popupshown listeners added');
  });
}

function updatePanel() {
  console.log('updatePanel called by a cascading popupshowing listener');
  const panel = getActiveView(sdkPanel);
  console.log('getActiveView(sdkPanel) is ', panel);
  // If we've run before, just bail.
  if (panel.getAttribute('minvid')) return console.log('We have fixed up this panel before. Bailing.');
  panel.setAttribute('minvid', true);
  panel.setAttribute('noautohide', true);
  panel.setAttribute('level', 'top');
  // TODO: this is why the arrow is visible on linux. close that bug with this commit.
  panel.removeAttribute('type');

  // just do the make-panel-draggable stuff inline real quick
  if (system.platform === 'winnt' || system.platform === 'darwin') {
    // 1. change some XUL attrs
    panel.setAttribute('backdrag', true);
    // 2. set display: -moz-stack on the panel, along with other stuff.
    // TODO: should I also apply these style changes to linux?
    panel.setAttribute('style', '-moz-appearance: none; border: 0; margin: 0; background: rgba(0,0,0,0); display: -moz-stack;');

    // 3. append the drag handle as the first child of the panel
    ...
    dragHandle.onmouseup = () => {
      ...
      // TODO: one nice thing about inlining make-draggable: just call the callback directly
      updatePosition();
    };
    panel.insertBefore(dragHandle, panel.firstElementChild);
  }

  // Reseat the element to get XBL to update the element based on attribute changes.
  panel.parentNode.replaceChild(panel, panel);
}

// TODO: Ensure we can abort at any point in this lengthy setup process.
function oldcreate(dimensions, panelOptions) {
  // Sequence of events:
...
function destroy() {
  if (sdkPanel) {
    ...
    // TODO: is it necessary to detach event listeners like this?
    getActiveView(sdkPanel).removeEventListener('popupshowing', updatePanel, true);
    getActiveView(sdkPanel).removeEventListener('popupshown', whenPanelReady.ready);
...
function updateWindow() {
  // Desperate moment. Just create, then show, to trigger all the popupshowing/shown callbacks.
  if (!sdkPanel) create();
  sdkPanel.show();
