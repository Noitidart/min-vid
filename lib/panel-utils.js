...
function create(dimensions, panelOptions) {
  ...
  console.log('before _create');
  sdkPanel = _create(dimensions, panelOptions);
  console.log('after _create, sdkPanel is: ', sdkPanel);
  ...
  //    to do all this.
  console.log('outside first _checkPanel');
  _checkPanel(() => {
    console.log('inside first _checkPanel');
    const xulPanel = getActiveView(sdkPanel);
    // what if we show/hide the panel? will that prompt the creation of the frame?
    sdkPanel.show();
    sdkPanel.hide();

    // TODO: what if we just don't wait for DOMContentLoaded?
    // what if we use a LONG setTImeout instead?
    setTimeout(() => {
      ...
      console.log('before second _checkPanel');
      _checkPanel(() => {
        console.log('after second _checkPanel');
        ...
        console.log('before worker-ready');
        sdkPanel.port.once('worker-ready', () => {
          console.log('after worker-ready');
        ...
        try {
          console.log('before worker-ready-check');
          sdkPanel.port.emit('worker-ready-check');
          console.log('after worker-ready-check');
...
function _checkPanel(cb, checkCount) {
  ...
  console.log('_checkPanel iteration ', checkCount);
...
function _getXulPanel() {
  ...
  const minVidBackgroundFrame = frame.backgroundFrame && frame.backgroundFrame.contentWindow &&
                                frame.backgroundFrame.contentWindow.location.toString().indexOf('min-vid') > -1;
  const minVidViewFrame = frame.viewFrame && frame.viewFrame.contentWindow &&
                            frame.viewFrame.contentWindow.location.toString().indexOf('min-vid') > -1;

  console.log('_getXulPanel: isSdkPopup? ', isSdkPopup);
  console.log('_getXulPanel: minVidBackgroundFrame? ', minVidBackgroundFrame);
  console.log('_getXulPanel: minVidViewFrame? ', minVidViewFrame);
