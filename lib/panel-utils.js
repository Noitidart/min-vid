...
const screenPosition = simpleStorage.storage.screenPosition || DEFAULT_SCREEN_COORDS;
...
function _create(dimensions, panelOptions) {
  ...
  // The first time we create the panel, we need to offset the screenPosition
  // by the window position to get to the correct screen coordinates.
  const currentWindow = Services.wm.getMostRecentWindow('navigator:browser');
  userPanelOptions.position.top = screenPosition.y - currentWindow.mozInnerScreenY;
  userPanelOptions.position.left = screenPosition.x - currentWindow.mozInnerScreenX;
