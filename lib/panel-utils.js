...
function create(dimensions, panelOptions) {
  ...
  // TODO: the panel styling error now is unavoidable, since DOMContentLoaded doesn't fire till the iframe is in the DOM.
  ...
  // We can't wait for popupshowing/shown to call whenPanelReady.ready(). That is
  // the callback that blocks calling panel.show to begin with. Just wait a turn.
  setTimeout(() => {
    console.log('panel-utils create: inside setTimeout callback. about to call whenPanelReady.ready');
    whenPanelReady.ready();
  });
  console.log('popupshowing listener added');
}
...
function updatePanel(evt) {
  ...
  // If we need to restyle the panel, just cancel this popupshowing event. We'll retrigger
  // another show at the end.
  evt.preventDefault();
  ...
  // try calling show again
  setTimeout(() => { sdkPanel.show(); });
}
...
function updateWindow(cb) {
  console.log('inside updateWindow');
  if (!sdkPanel) {
    console.log('sdkPanel not found. creating');
    create();
    console.log('****** inside updateWindow, after calling create(), sdkPanel is now ', sdkPanel);
    // Wait a turn for the panel to be set up??
  ...
  // Panel's already in the DOM. No need to wait a turn.
  cb();
}
