  ...
  _checkPanel(() => {
    ...
    _checkPanel(() => {
      ...
      sdkPanel.port.once('worker-ready', () => {
        ...
        // TODO: let's see if setting the screen position at this point works.
        redraw();
...
function redraw() {
  ...
  console.log('redraw called. moving to (', position.x, ', ', position.y, ')');
  const xulPanel = getActiveView(sdkPanel);
  // le sigh. the last resort.
  setTimeout(() => {
    console.log('attempting to move to: ', position.x, ', ', position.y);
    const xulPanel = getActiveView(sdkPanel);
...
function updateWindow() {
  ...
  if (currentWindow !== panelWindow) {
    destroy();
    const panel = create();
    // TODO: if we could position it correctly when we create() it, this would be unnecessary
...
function updatePosition() {
  ...
  */
  screenPosition.x = clientRect.left + windowX;
  screenPosition.y = clientRect.top + windowY;
}
