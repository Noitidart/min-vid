...
function launchVideo(opts) {
  let panelExists = !!panelUtils.getPanel();
  let panel;
  ...
  panelUtils.whenPanelReady(() => {
    ...
    if (!opts.src) {
      opts.getUrlFn(id, function(err, streamUrl) {
        ...
        const panel = panelUtils.getPanel();
        if (err) return console.error('LaunchVideo failed to get the streamUrl: ', err);
        if (!panel) return console.error('LaunchVideo lost the panel while waiting for the streamUrl.');
