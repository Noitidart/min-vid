...
function launchVideo(opts) {
  console.log('inside launchVideo');
  // UpdateWindow might create a new panel, so do the remaining launch work
  // asynchronously. Pass the callback directly in.
  panelUtils.updateWindow(() => {
    console.log('lib/launch-video: inside whenPanelReady callback');
    const panel = panelUtils.getPanel();
    if (!panel) { console.log('lib/launch-video: panelUtils.getPanel() is ', panel); }
    // opts {url: url,
    ...
    console.log('launchVideo: about to call panel.port.emit');
    try {
      panel.port.emit('set-video', {domain: opts.domain, id: id, src: opts.src || ''});
    } catch (ex) {
      console.error('launchVideo: panel.port.emit failed: ', ex);
    }

    console.log('launchVideo: about to panel.show');
    try {
      panel.show();
    } catch (ex) {
      console.error('launchVideo: panel.show failed: ', ex);
    }

    if (!opts.src) {
      opts.getUrlFn(id, function(err, streamUrl) {
        console.log('launchVideo: inside getUrlFn callback');
        // Be careful not to close over the outer panel reference.
        // If the panel went away while waiting for the stream, just give up.
        const panel = panelUtils.getPanel();
        if (!panel) { console.log('launchVideo: inner getPanel returned nothing: ', panel); }
