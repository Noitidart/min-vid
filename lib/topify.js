...
function _winntTopify(winPtrStr) {
  ...
  // Try HWND_TOP because HWND_TOPMOST puts _all_ Firefox windows in the topmost
  // layer (so all FF windows are always above other apps)
  const didTopify = ostypes.API('SetWindowPos')(winPtr, ostypes.CONST.HWND_TOP, 0, 0, 0, 0, ostypes.CONST.SWP_NOSIZE | ostypes.CONST.SWP_NOMOVE);
