...
DraggableElement.prototype.handleEvent = function(aEvent) {
  if (aEvent.type === 'mousedown' && /^gtk/i.test(AppConstants.MOZ_WIDGET_TOOLKIT)) {
    ...
    const winPtrStr = getNativeHandlePtrStr(this._window);
    ...
    let screenX = ostypes.TYPE.gint();
    let screenY = ostypes.TYPE.gint();
    // noitidart put together an example that shows how to get the mouse coords
    // and which button was pressed from C-level code: 
    // https://github.com/Noitidart/ostypes_playground/commit/55ec7ccf
    // however, it looks like the existing firefox code obtains this info from the
    // DOM mousedown event. so, let's try that way first.
    // TODO: make sure the button is still pressed down. seems (based on the Gecko
    // code) like this might throw if not.

    // we need to convert these values to whatever gdk expects.
    // have a look at nsWindow::BeginMoveDrag:
    // - DevicePixelsToGdkCoordRoundDown is used to transform clientX and clientY.
    // - GetDragInfo is also involved. looks like it calls WidgetToScreenOffset to
    //   get the native coords of the widget, then adds in the mouse pointer offset
    //   from the event?
    screenX = parseInt(aEvent.clientX);
    screenY = parseInt(aEvent.clientY);
    let timestamp = ostypes.TYPE.gint(ctypes.UInt32(aEvent.timestamp));
    // 3. call the method
    ostypes.API('gdk_window_begin_move_drag')(gdkWin, button, screenX, screenY, timestamp);
  }
