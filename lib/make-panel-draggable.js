...
function makePanelDraggable(sdkPanel) {
  ...
  dragHandle.onmouseup = () => {
    ...
    const { docBottom: bottom, docLeft: left } = doc.documentElement.getBoundingClientRect();
    const { panelBottom: bottom, panelLeft: left } = panel.getBoundingClientRect();

    const bottomOffset = docBottom - panelBottom;
    const leftOffset = docLeft - panelLeft;
    
    // Store the offset to preserve location when resizing.
    panelCoords.bottomOffset = bottomOffset;
    panelCoords.leftOffset = leftOffset;
  };
